...
export default function EditAuctionPage({ params }: EditAuctionProps) {
  const auction = disposalAuctions.find(a => a.id === params.id) || disposalAuctions[0]
  
  const [formData, setFormData] = useState({
    title: auction.title,
    estimatedValue: auction.estimatedValue.replace(/[^0-9]/g, ''),
    startDate: new Date(auction.startDate) as Date | null,
    endDate: new Date(auction.endDate) as Date | null,
    status: auction.status,
  })
  
  const [items, setItems] = useState<AuctionItem[]>([
    { id: "ITEM-1", name: "Old Office Computers", description: "5 desktop computers", condition: "Fair", estimatedValue: 25000 },
  ...
  const handleSubmit = () => {
    console.log("Updating auction:", { ...formData, description: editor?.getHTML(), items })
  }

  return (
    <Stack gap="lg">
      <Group>
        <Link href={`/application/disposal-auctions/internal/${params.id}`}>
          <ActionIcon variant="subtle" size="lg">
            <IconArrowLeft size={20} />
          </ActionIcon>
        </Link>
        <div>
          <Title order={2}>Edit Auction</Title>
          <Text c="dimmed" size="sm">Update auction details and items</Text>
        </div>
      </Group>
      ...
      <Card shadow="sm" padding="lg" radius="md" withBorder>
        <Stack gap="md">
          ...
          <Select
            ...
            data={["Draft", "Open for Bidding", "Bidding Closed", "Cancelled"]}
          />
